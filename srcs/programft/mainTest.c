...
/*   Updated: 2023/05/23 03:27:53 by sgerace          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_everything(t_cub* cub, int key)
{
	if (draw_minimap(cub))
...
{
	// printf("DIRX: %f DIRY: %f POSX: %f POSY: %f\n", cub->player_pos.dirx, cub->player_pos.diry, cub->player_pos.x, cub->player_pos.y);
	...
	for (int x = 0; x <= WIN_SIZE_W; x++)
	{
		...
		while (distance_to_wall < 100)
		{
			...
			if (cub->map[((int)test_x)][((int)test_y)] == '1')
			{
				if (x == WIN_SIZE_W / 2)
					cub->player_pos.wd = distance_to_wall;
				break ;
			}
